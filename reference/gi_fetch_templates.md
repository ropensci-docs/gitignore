...
- copy_to_clipboard:
  ...
  Logical. Should the returned template(s) be copied to the clipboard?
  Otherwise, it will be printed in the console. Default is FALSE.

- append_gitignore:

  Logical. Should the .gitignore be modified to include the returned
  template(s)?

- gitignore_file:

  The path of the .gitignore file to be modified. By default, it will
  try to find it in the current package/project using
  \`here::here(".gitignore")\`.
